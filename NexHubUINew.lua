...
-- Custom Logo Image
local nexhub_logo     = "rbxassetid://82006436469351"
...
local function is_font_descriptor(value)
    return type(value) == "table" and value.__nexhub_font == true
end
...
function nexhub_ui.new(config)
    cleanup_previous_instance()

    local self = setmetatable({}, nexhub_ui)
    ...
    self._autoConfigName = sanitize_config_name((self.config.Name or "NexHub") .. "_last")
...
function nexhub_ui:_TrackConnection(conn)
    if conn then
...
function nexhub_ui:RegisterControl(flag, getter, setter)
...
function nexhub_ui:_RegisterRefreshJob(interval, isAliveFn, stepFn)
...
function nexhub_ui:_StepRefreshJobs(dt)
...
function nexhub_ui:_BuildAutoConfigSnapshot()
...
function nexhub_ui:_TryAutoSaveConfig(forceSave)
...
function nexhub_ui:_TryAutoLoadConfig(forceAttempt)
...
function nexhub_ui:_RefreshViewportMetrics()
...
function nexhub_ui:_InvalidateESPPreviewProjection(clearCache)
    self._espPreviewProjectionDirty = true
    if clearCache then
        self._espPreviewProjectionCache = nil
    end
...
function nexhub_ui:_EnsureBlurEffect()
    if self._blurEffectRef and self._blurEffectRef.Parent then
        return self._blurEffectRef
    end
    local existingBlur = lighting:FindFirstChild("NexHubBlurEffect")
...
function nexhub_ui:_SetBlurActive(active, instant)
...
function nexhub_ui:_ClearSnowflakes()
...
function nexhub_ui:_SpawnSnowflake()
...
function nexhub_ui:_SpawnRainDrop()
    if not self.snow_layer or not self.snow_layer.Parent then
        return
    end
    local width = self._cachedViewportWidth or 200
...
function nexhub_ui:_SpawnRainSplash(xPosition, yPosition)
    if not self.snow_layer or not self.snow_layer.Parent then
        return
    end
...
function nexhub_ui:_SpawnStarParticle()
    if not self.snow_layer or not self.snow_layer.Parent then
        return
    end
    local width = self._cachedViewportWidth or 200
...
function nexhub_ui:_SetSnowEnabled(enabled)
...
function nexhub_ui:_SetOverlayMode(mode)
...
function nexhub_ui:_SetBackgroundEffectsEnabled(enabled)
...
function nexhub_ui:_SetNameHidden(enabled)
...
function nexhub_ui:_SetTextGradientEnabled(enabled)
...
function nexhub_ui:_AnimateTextGradients(dt)
    if not self._uiVisualSettings.TextGradient then
        return
    end
...
function nexhub_ui:SetFontPreset(index)
...
function nexhub_ui:_RefreshAccentCore()
    local accent = self.config.AccentColor
...
function nexhub_ui:_ApplyOpenCloseVisuals(instant)
...
function nexhub_ui:SetESPProvider(providerFn)
...
function nexhub_ui:SetESPData(previewData)
...
function nexhub_ui:SetESPPreview(enabled)
...
function nexhub_ui:_ResolveESPPreviewWalkAnimationId(sourceCharacter)
...
function nexhub_ui:_StartESPPreviewWalkAnimation(sourceCharacter)
...
function nexhub_ui:_CreateESPPreviewFallbackCharacter()
...
function nexhub_ui:_DestroyESPPreviewCharacter()
    if self._espPreviewWalkTrack then
        pcall(function()
            self._espPreviewWalkTrack:Stop(0.1)
        end)
...
function nexhub_ui:_EnsureESPPreviewCharacter()
...
function nexhub_ui:_CreateESPPreviewPanel()
...
function nexhub_ui:_ResolveESPPreviewState()
    local state = {
        Enabled = true,
        ShowBox = false,
        ShowHealth = false,
...
function nexhub_ui:_ApplyESPPreviewPartVisualState(showChams, previewColor, chamsTransparency)
...
function nexhub_ui:_UpdateESPPreview(dt)
...
function nexhub_ui:SetSmoothScroll(scrollFrame, smoothSpeed)
...
function nexhub_ui:SetSearchFilter(rawQuery)
...
function nexhub_ui:SaveConfig(fileName)
...
function nexhub_ui:LoadConfig(fileName)
...
function nexhub_ui:SetToggleVisible(visible)
...
function nexhub_ui:SetToggleKey(keyCode)
    self.toggleKeyCode = keyCode
end

function nexhub_ui:BuildToggleButton()
...
function nexhub_ui:BuildWatermark()
    ...
    create("ImageLabel", {
        Image = nexhub_logo, BackgroundTransparency = 1,
        AnchorPoint = Vector2.new(0, 0.5),
        Position = UDim2.new(0, 8, 0.5, 0),
...
function nexhub_ui:_UpdateWatermark(dt)
...
function nexhub_ui:_ResizeLayout(newWidth, newHeight)
...
function nexhub_ui:BuildMainFrame()
...
function nexhub_ui:BuildNotificationHolder()
...
function nexhub_ui:Notify(config)
    config = config or {}
    ...
    config.Icon = config.Icon or nexhub_logo or "rbxassetid://10709768141"
...
function nexhub_ui:AddSection(config)
    config = config or {}
...
function nexhub_ui:SetAccentColor(color)
...
function nexhub_ui.Demo()
    local lib = nexhub_ui.new({
        Name = "NexHub Demo",
    ...
    misc_group:AddLabel({Name = "v1.0.0 - NexHub Demo"})
